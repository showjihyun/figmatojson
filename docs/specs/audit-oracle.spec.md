...
- 🟢 **Schema enum rename**: `stackPrimaryAlignItems` 의 `SPACE_EVENLY`
  (kiwi value=3) ↔ Figma 현재 명명 `SPACE_BETWEEN`. 같은 binary value 를
  schema 버전 간에 rename 한 결과 — round-trip 시 binary value 보존이라
  semantic 영향 없음.
...
- 🟢 **Rotation matrix edge**: `±90°` rotation 등 matrix m00 이 거의 0
  근방에서 atan2 derivation 이 약간 어긋남 (signal 2건 / 18,304 비교).
