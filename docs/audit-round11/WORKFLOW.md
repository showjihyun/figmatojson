...
페이지 슬러그: `design-setting`, `web`, `mobile`, `dash-board`, `icons`.
`Internal Only Canvas` 는 `.fig` 소스에서 `visible: false` 인 디자이너
스크래치패드 페이지 (VARIABLE/BRUSH/SYMBOL 정의 보관용; 다른 페이지의
INSTANCE 가 참조). Figma UI/REST API 모두 노출하지 않으므로 round-11
부터 인벤토리에서 제외. 컴포넌트 슬러그는 `<이름>-<sessionID_localID>` 형식.
